...
public class WindowUtils {
    ...
    public static void codeWait(final TextView tvGetCode, int miCnt) {
        codeWait(tvGetCode, "%ss", miCnt);
    }

    //60秒倒计时
    public static void codeWait(final TextView tvGetCode, final String formatStr, final int miCnt) {
        tvGetCode.setEnabled(false);
        new CountDownTimer(miCnt * 1000, 1000) {

            @Override
            public void onTick(long millisUntilFinished) {
                tvGetCode.setText(String.format(formatStr, millisUntilFinished / 1000));
            }
